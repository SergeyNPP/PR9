...
// числа matrix[i, j] с двумя знаками после запятой (f2) и добавления символа табуляции (\t)
//  после каждого элемента матрицы. Таким образом, каждый элемент матрицы будет разделен 
//  символом табуляции при выводе.
...
double[,] array = new double[m,n];
    for (int i = 0; i < m; i++)
    {
      for (int j = 0; j < n; j++)
      {
        array[i,j] =10*new Random().NextDouble();
      }
    }
    return array;
  }
